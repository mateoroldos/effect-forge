...
Server-side SvelteKit operations enter through the request-owned runtime in `apps/web/src/lib/server/runtime.ts`. `hooks.server.ts` builds it from the request and Worker environment, shares it through `event.locals`, and awaits disposal after SvelteKit resolves the response. Better Auth's promise-based Drizzle adapter and application persistence's Effect Drizzle client share one request-owned PostgreSQL pool. Authentication is bound to that request and memoized as one Effect result; a separate HTTP request receives a new runtime, bypasses Better Auth's cookie cache, and reads the authoritative session again. Better Auth's configured background-capable operations use its default awaited execution, so provider work settles before that pool closes. Cloudflare does not expose an isolate shutdown hook, and Hyperdrive discourages global database clients, so the runtime must not outlive the request. Principal and workspace context remain explicit application-operation inputs rather than runtime services.
