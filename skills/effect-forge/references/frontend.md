...
5. Use the existing hover setting in `app.html` for route code and layout data. Keep `experimental.forkPreloads` disabled: cold-hover speculative rendering can crash Kit's route-data getter on the tested versions. Component-owned list queries start on navigation, not hover. Do not add manual hover-fetch handlers, duplicate list snapshots, or another cache; remote queries own their live data and deduplicate while in active use.
